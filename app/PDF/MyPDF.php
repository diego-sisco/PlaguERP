...
class MyPDF extends TCPDF
{
    ...
    public function Services()
    {
        ...
        if ($order->services->isNotEmpty()) {
            foreach ($order->services as $service) {
                if ($service->prefix == 2) {
                    $details = ServiceDetails::where('service_id', $service->id)->where('contract_id', $order->contract_id)->first()->details;
                    ...
                    $this->SetXY($x, $newY);
                    $this->SetFont('helvetica', '', 9);
                    $this->writeHTML($details, true, false, true, false, '');
    ...
    public function Products()
    {
        ...
        if ($order_products->isNotEmpty()) {
            ...
            foreach ($order_products as $order_product) {
                ...
                $x = $this->GetX();
                $this->MultiCell($width_td, 6, ($order_product->amount ?? 'N/A') . ' ' . ($metric ?? ''), 0, 'L', true);
                $this->SetXY($x + $width_td, $y);

                $x = $this->GetX();
                $this->MultiCell($width_td, 6, $order_product->product->lot->lot_number ?? 'N/A', 0, 'L', true);
            ...
            foreach ($floorplans as $floorplan) {
                ...
                foreach ($points as $point) {
                    ...
                    foreach ($selected_devices as $device) {
                        ...
                        $this->SetXY($newX, $y);
                        $this->MultiCell($width_td, $step, implode(', ', $reviews), 0, 'C', true);
                        $newX = $newX + $width_td;
                        ...
                        foreach ($questions as $question) {
                            ...
                            $this->MultiCell($width_td, $step, $incident->answer ?? 'S', 0, 'C', true);
    ...
    {
        $order = Order::find($this->orderId);
        $width = ($this->widthPage - $this->margin) / 3;
        $step = 10;
        ...
        $y = $this->GetY() + $step * 1.5;
        $this->SetXY($x, $y);
        $this->Cell($width, 5, 'Recibi del cliente', 0, 0, 'C');
