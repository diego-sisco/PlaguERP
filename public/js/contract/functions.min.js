...
function createService(service) {
    ...
    var html = `
            <tr>
                ...
                <td class="text-center" id="count-service${
                    service.id
                }">${count}</td>
...
function createServicesList() {
    ...
    if (typeof contract_services != 'undefined') {
        contract_services.forEach((item, index) => {
            settings.push({
                index: index,
...
function setSettings() {
    ...
    if (pos != -1) {
        ...
        if (contract_data[pos].settings.length > 0) {
            contract_data[pos].settings.forEach((setting) => {
                setting.dates = [];
                createDates(setting, startDate, endDate);
                html += createSetting(setting, pos);
            });
            $("#settings").html(html);
        }
    }
...
function createSetting(setting, pos) {
    var execution =
        execution_frecuencies.find((exec) => exec.id == setting.frequency)
            .name ?? "S/A";
    var interval = intervals[setting.interval - 1];
    var html = `
        <tr>
            <th scope="row">${setting.index + 1}</th>
            <td>${execution}</td>
            <td>${interval}</td>
            <td>${setting.days}</td>
            <td>${setting.dates.length}</td>
            <td>
            <button type="button" class="btn btn-warning btn-sm" onclick="showPreview(${pos}, ${
        setting.index
    })"><i class="bi bi-calendar-fill"></i></button>
                <button class="btn btn-danger btn-sm" onclick="deleteSetting(${pos}, ${
        setting.index
