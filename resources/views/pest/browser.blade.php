<form class="input-group d-flex" method="GET" action="{{ route('pest.search', ['page' => 1]) }}"
    ...
    <button type="submit" class="btn btn-success rounded-0 rounded-end-2" data-bs-toggle="tooltip"
        data-bs-placement="bottom" data-bs-title=" {{ __('pest.tips.search') }}"> {{ __('buttons.search') }} </button>
</form>
