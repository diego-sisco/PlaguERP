<form id="create_service_form" class="form" method="POST" action="{{ route('rotation.update', ['id' => $rotation_plan->id]) }}"
    ...
    <div class="row mb-2">
        ...
        <div class="col-4 mb-3">
            ...
            <input type="text" class="form-control border-secondary border-opacity-50"
                value="{{ $rotation_plan->customer->name }}" disabled />
        </div>
        <div class="col-2 mb-3">
            <label class="form-label is-required">Contracto asignado</label>
            <input type="text" class="form-control border-secondary border-opacity-50"
                value="[{{ $rotation_plan->contract->id }}] {{ date('d-m-Y', strtotime($rotation_plan->contract->startdate)) }} : {{ date('d-m-Y', strtotime($rotation_plan->contract->enddate)) }}"
                disabled />
        </div>
        <div class="col-1 mb-3">
            ...
            <input type="number" class="form-control border-secondary border-opacity-50" id="no-review"
                name="no_review" value="{{ $rotation_plan->no_review }}" min="1" />
        </div>
    </div>
    <div class="row mb-2">
        ...
        <div class="col-6 mb-3">
            ...
            <input type="text" class="form-control border-secondary border-opacity-50" id="name" name="name"
                placeholder="Nombre del plan de rotación" value="{{ $rotation_plan->name }}" required />
        </div>
        <div class="col-2 mb-3">
            <label class="form-label is-required">Código </label>
            <input type="text" class="form-control border-secondary border-opacity-50" id="code" name="code" value="{{ $rotation_plan->code }}"
                required />
        </div>
        <div class="col-2 mb-3">
            ...
            <input type="date" class="form-control border-secondary border-opacity-50" id="authorization-at"
                name="authorization_at" value="{{$rotation_plan->authorization_at}}" required />
        </div>
    </div>
    ...
    <div class="row mb-2">
        ...
        <div class="col-2 mb-3">
            ...
            <input type="date" class="form-control border-secondary border-opacity-50" id="startdate"
                name="start_date" value="{{ $rotation_plan->start_date }}" required />
        </div>
        <div class="col-2 mb-3">
            ...
            <input type="date" class="form-control border-secondary border-opacity-50" id="enddate" name="end_date"
                value="{{ $rotation_plan->end_date }}" required />
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-12 mb-3">
            ...
            <input type="text" class="form-control border-secondary border-opacity-50" id="importart-text"
                name="importart_text" value="{{ $rotation_plan->important_text }}" />
        </div>
        <div class="col-12 mb-3">
            <label class="form-label">Notas </label>
            <textarea class="form-control border-secondary border-opacity-50" id="notes" name="notes" rows="5">{{ $rotation_plan->notes }}</textarea>
        </div>
    </div>

    <input type="hidden" id="url-search-product" value="{{ route('rotation.search.product') }}" />
    <input type="hidden" name="contractId" value="{{ $rotation_plan->contract->id }}" />
...
<div class="modal fade" id="dateModal" tabindex="-1" aria-labelledby="dateModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary"
                    onclick="setDates()">{{ __('buttons.update') }}</button>
                <button type="button" class="btn btn-danger"
                    data-bs-dismiss="modal">{{ __('buttons.cancel') }}</button>
            </div>
        </div>
...
<script>
    const fetched_products = @json($product_data);
</script>