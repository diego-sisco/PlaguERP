<form method="POST" class="form" action="{{ route('warehouse.update', $warehouse->id) }}" enctype="multipart/form-data">
    ...
    <div class="row">
        <div class="col-6 mb-3">
            ...
            <input type="text" class="form-control border-secondary border-opacity-25" id="name" name="name" value="{{ $warehouse->name }}" required>
        </div>
        
        <div class="col-auto mb-3">
            <label for="receive_material" class="form-label is-required">Permitir recepciones de material </label>
            <select class="form-select border-secondary border-opacity-25" id="receive-material" name="receive_material">
        ...
        <div class="col-auto mb-3">
            ...
            <select class="form-select border-secondary border-opacity-25" id="active" name="active">
    ...
    <div class="row">
        <div class="col-4 mb-3">
            ...
            <input type="number" class="form-control border-secondary border-opacity-25" id="phone" name="phone" value="{{ $warehouse->phone }}" placeholder="0000000000" autocomplete="off" min="0">
        </div>
        <div class="col-4 mb-3">
            <label for="address" class="form-label is-required">{{ __('customer.data.address') }}</label>
            <input type="text" class="form-control border-secondary border-opacity-25" id="address" name="address" value="{{ $warehouse->address }}" placeholder="#00 Col. Example" required>
        </div>
        <div class="col-2 mb-3">
            <label for="zip_code" class="form-label is-required">{{ __('customer.data.zip_code') }} </label>
            <input type="number" min=10000 max=99999 class="form-control border-secondary border-opacity-25" id="zip_code" name="zip_code" value="{{ $warehouse->zip_code}}" placeholder="00000" required>
        </div>
        <div class="col-2 mb-3">
            <label for="state" class="form-label is-required">{{ __('customer.data.state') }} </label>
            <select class="form-select border-secondary border-opacity-25" id="state" name="state" onchange="load_city()" required>
    ...
    <div class="row">
        <div class="col-12 mb-3">
            ...
            <textarea type="text" class="form-control border-secondary border-opacity-25" id="observations" name="observations" rows="4">{{ $warehouse->observations }}</textarea>
        </div>
    </div>
